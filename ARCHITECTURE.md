...
- Streamlit Community Cloud sleeps after inactivity. To spare visitors a cold
  "wake-up", an external **[cron-job.org](https://cron-job.org)** monitor pings the
  app every 5 minutes (with "follow/accept redirects" on, so Streamlit's `303`
  session handshake counts as success). App: <https://drjhagpt.streamlit.app>
