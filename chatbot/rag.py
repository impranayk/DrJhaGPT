"""Lightweight retrieval over Dr. Pranay Jha's website content.

Replaces IBM Watson Discovery. Uses fastembed (ONNX embeddings, no PyTorch)
and a plain NumPy cosine-similarity search over a small, prebuilt index.
"""
import json
...
def format_context(results: List[Dict]) -> str:
    ...
    blocks = []
    for r in results:
        blocks.append(f"[{r.get('title', 'Article')}]\n{r.get('text', '')}")
    return "\n\n".join(blocks)
